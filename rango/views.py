...
# Helper functions - Cookies handling

def visitor_cookie_handler(request):
    # Get the number of visits to the site. We use the COOKIES.get()
    # function to obtain the visits cookie. If the cookie exists, the 
    # value returned is casted to an integer. If the cookie does not 
    # exist, the the default value of 1 is used.
    ...
    # Is it is been more than a fday since the last visit...
    if (datetime.now() - last_visit_time).days > 0:
        visits = visists + 1
        # Update the last visit cookie now that we updated the count
